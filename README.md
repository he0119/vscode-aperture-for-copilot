...
把 Aperture 模型接入 GitHub Copilot Chat 的模型选择器。本扩展支持 OpenAI Chat Completions、OpenAI Responses 和 Anthropic Messages 三种 API 协议。
...
- 手动模型可通过 `apiProtocol` 选择 `chat-completions`、`responses` 或 `anthropic-messages`。
...
`responses` 会请求 `/v1/responses`，`anthropic-messages` 会请求 `/v1/messages`。自动发现的模型默认使用 `chat-completions`，可通过同 ID 的模型配置覆盖协议。
